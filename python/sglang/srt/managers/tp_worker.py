...
class TpModelWorker:
    ...
    def forward_batch_generation(
        ...
        if self.pp_group.is_last_rank:
            ...
            if skip_sample:
                next_token_ids = None
                # For scoring requests, we still need to compute token_ids_logprobs even when sampling is skipped
                if any(token_ids is not None for token_ids in model_worker_batch.token_ids_logprobs):
                    # Compute only the logprobs for requested token IDs without full sampling
